...
void System::Receiver::Interpret::Recalibration::HandleMessage(const SystemContext& context, Component::Receiver& receiver)
{
...
void System::Receiver::Interpret::Recalibration::Update(entt::registry& registry, const float deltaTime)
{
	const entt::entity entity = entt::get_single<Component::Radar>(registry);
	auto& radar = registry.get<Component::Radar>(entity);
	auto& toggle = registry.get<Component::Action::Toggle>(entity);

	if (!radar.isRecalibrating) return;;
	if (radar.recalibrationTimeLeft <= 0.0f)
	{
		return Completed(radar, toggle);
	}
	radar.recalibrationTimeLeft -= deltaTime;
}


void System::Receiver::Interpret::Recalibration::ConfirmCommand(const SystemContext& context, Component::Receiver& receiver)
{
	// TODO: Add response
...
void System::Receiver::Interpret::Recalibration::Completed(Component::Radar& radar, Component::Action::Toggle& toggle)
{
